# 今回行ったこと — ステガノグラフィー解析（レポート）

## 概要（導入）

ステガノグラフィー（steganography）は、データを第三者に気付かれないように別のメディア（画像・音声・動画など）に埋め込む技術である。暗号が「内容を読めないようにする」ことを目的とするのに対し、ステガノグラフィーは「存在そのものを隠す」ことを目的とする点が異なる。ステガノグラフィーを解析・発見する人は一般に **ステガノグラファー（steganographer）** や **ステガ解析者（stego analyst）** と呼ばれる。

本稿では、私が picoCTF の課題 `red.png` に対して行った解析手順、使用したツール、得られた知見・教訓をまとめる。
...
まずファイルの基本情報を確認した。PNGは可逆圧縮（DEFLATE）を使うが、画像処理ライブラリや専用ツールを使うことでピクセル値やPNGチャンクを扱える。
...
`strings` コマンド等でファイル内の可視文字列を確認したところ、PNGの tEXt チャンクに詩（Poem）が含まれていることが分かった。

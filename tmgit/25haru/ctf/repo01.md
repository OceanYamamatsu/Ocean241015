...
Webアプリケーションにおけるファイルアップロード機能は、利便性の一方で重大なセキュリティリスクを伴う。中でも、PHPスクリプトをアップロードすることで任意のコードが実行される脆弱性（PHPインジェクション）は、攻撃者によるサーバ乗っ取りにつながることもある。本研究では、
<!-- CTF問題「n0s4n1ty 1」を題材に、実際に脆弱性の再現・検証を行い、その仕組みと対策について考察する。 -->

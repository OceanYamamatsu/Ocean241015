...
FROM PurchaseSlip, Product
WHERE PurchaseSlip.PrdID = Product.ID;
...
FROM SalesInfo, Product
WHERE SalesInfo.PrdID = Product.ID
GROUP BY SalesInfo.PrdID;
...
FROM PurchaseSlip, Product, Supplier
WHERE PurchaseSlip.PrdID = Product.ID AND Product.SupID = Supplier.ID;
...
FROM SalesSlip, SalesInfo, Product, Customer
WHERE SalesSlip.ID = SalesInfo.SSlpID 
  AND SalesInfo.PrdID = Product.ID 
  AND SalesSlip.CustID = Customer.ID;
...
FROM Product, Stock, SalesInfo
WHERE Product.ID = Stock.PrdID
  AND (Product.ID = SalesInfo.PrdID OR SalesInfo.PrdID IS NULL)
...
FROM Product, Stock
WHERE Product.ID = Stock.PrdID AND Stock.Stk = 0;
